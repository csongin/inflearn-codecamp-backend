...
const createTree = function (folder, node) {
  // 여기에서 작업하세요.
  for (let el of folder) {
    const folderName = el.name;
    const folderFile = el.file;
    const lowerFolder = el.folder;
    ...
    // 하위 file 생성
    if (folderFile) {
      for (let el of folderFile) {
        const newFile = document.createElement("li");
        newFile.textContent = el;
    ...
    // 하위 folder 생성
    if (lowerFolder) {
      for (let el of lowerFolder) {
        createTree([el], newToggle);
      }
    } else {
      return;
    }
